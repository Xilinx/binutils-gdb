...
/* The entries in the second procedure linkage table for MPX look like
   this.  */

static const bfd_byte elf_x86_64_mpx_plt_entry[PLT_ENTRY_SIZE] =
{
		  ...
		  if (htab->plt_bnd == NULL)
		    {
		      ...
		      switch (sizeof (elf_x86_64_mpx_plt_entry))
			{
			case 8:
			  plt_bnd_align = 3;
			  break;
...
elf_x86_64_allocate_dynrelocs (struct elf_link_hash_entry *h, void * inf)
{
  ...
  if (h->type == STT_GNU_IFUNC
	  ...
	  || WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, 0, h))
	{
	  ...
	  s->size += plt_entry_size;
	  if (eh->has_bnd_reloc)
	    bnd_s->size += sizeof (elf_x86_64_mpx_plt_entry);
				    ...
				    struct bfd_link_info *info)
{
  ...
  if (htab->plt_bnd != NULL)
    elf_section_data (htab->plt_bnd->output_section)
      ->this_hdr.sh_entsize = sizeof (elf_x86_64_mpx_plt_entry);
